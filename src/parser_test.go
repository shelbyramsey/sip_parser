...
func TestParseMsg(t *testing.T) {
    ...
    s := ParseMsg(m)
    if s.Error != nil {
	t.Errorf("[TestParseMsg] Error parsing msg. Recevied: ", s.Error)
    }
    if len(s.Body) == 0 {
...
func TestGetCallingParty(t *testing.T) {
    ...
    if err != nil {
	t.Errorf("[TestGetCallingParty] Err with GetCallingParty.  rcvd: ", err)
    }
    ...
    if err != nil {
	t.Errorf("[TestGetCallingParty] Err with GetCallingParty on paid.  rcvd: ", err)
    }
    if s.CallingParty == nil {
    ...
    if err != nil {
	t.Errorf("[TestGetCallingParty] Err calling GetCallingParty on default.  rcvd: ", err)
    }
    if s.CallingParty == nil {
