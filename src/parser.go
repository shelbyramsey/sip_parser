...
func (s *SipMsg) parseRecordRoute(str string) {
	...
	if left < right {
	    ...
	    if u.Error != nil {
		s.Error = errors.New("parseRecordRoute err: received err parsing uri: " + fmt.Sprintf("%s", u.Error))
		return
	    }
...
func (s *SipMsg) parseRoute(str string) {
	...
	if left < right {
	    u := ParseURI(cs[rt][left + 1:right])
	    if u.Error != nil {
		s.Error = errors.New("parseRoute err: received err parsing uri: " + fmt.Sprintf("%s", u.Error))
		return
	    }
	    if s.Route == nil {
		s.Route = []*URI{u,}
...
func (s *SipMsg) parseStartLine(str string) {
    ...
    if s.StartLine.Error != nil {
        s.Error = errors.New("parseStartLine err: received err while parsing start line: " + fmt.Sprintf("%s",s.StartLine.Error))
    }
}
