...
func TestFrom(t *testing.T) {
    ...
    sm.parseFrom(str)
    if sm.Error != nil {
	t.Errorf("[TestFrom] Error parsing from hdr: \"Unknown\" <sip:5554441000@0.0.0.0;user=phone;noa=national>;tag=dd737a8-co7387-INS002.  Received err: ", sm.Error)
    }
    ...
    sm.parseFrom(str)
    if sm.Error != nil {
	t.Errorf("[TestFrom] Error parsing from hdr: \"<sip:5554441000@0.0.0.0;user=phone;noa=national>;tag=dd737a8-co7387-INS002\".  Received err: ", sm.Error)
    }
    if sm.From.Name != "" {
    ...
    sm.parseFrom(str)
    if sm.Error != nil {
	t.Errorf("[TestFrom] Error parsing from hdr: sip:+12125551212@phone2net.com;tag=887s. Received err: %s", sm.Error)
    }
    if sm.From.Tag != "887s" {
